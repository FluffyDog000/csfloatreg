...
У firstmail **два разных API**, и ключи у них разные:

| API | База | Заголовок | Ключ |
|---|---|---|---|
| панельный | `https://firstmail.ltd/api/v1` | `Authorization: Bearer <ключ>` | из `/panel/api/keys/` |
| «рыночный» | `https://api.firstmail.ltd/v1` | `X-API-KEY: <ключ>` | отдельный, для купленных в шопе ящиков |

По умолчанию настроен панельный — именно такой ключ выдаёт панель. Для рыночного
поставь `auth_header: X-API-KEY`, `auth_prefix: ''` и `base_url: https://api.firstmail.ltd/v1`.
Список писем ни один из них не отдаёт (`/market/get/messages` → 404), поэтому
`messages_path: null` — читается только последнее письмо ящика.
