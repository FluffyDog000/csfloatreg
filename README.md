...
Рабочий адрес (проверен разведкой 09.2026): `https://api.firstmail.ltd/v1` +
`/market/get/message` — сервис отдаёт **только последнее письмо**, списка писем у него
нет, поэтому `messages_path: null`.
