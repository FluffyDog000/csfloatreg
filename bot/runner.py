...
class Runner:
    ...
    async def _finish(
        ...
        self._mark_account(login, status, stage, error)
    ...
    def _mark_account(self, login: str, status: str, stage: str, error: str) -> None:
        """Итог прогона виден и в менеджере профилей: там свои пометки на тех же аккаунтах."""
        mark = "done" if status == Status.DONE else "bad"
        note = "" if mark == "done" else f"{stage}: {error}"[:200]
        try:
            self.bindings.set_status(login, mark, note)
        except Exception as exc:  # noqa: BLE001 — пометка не важнее прогона
            self.log.debug("Не удалось записать статус в bindings.json: %s", exc)
