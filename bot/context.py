...
class AccountContext:
    ...
    bindings: Any = None        # BindingStore — общая с менеджером память об аккаунте
    ...
    def remember(self, field: str, value: str) -> None:
        """Кладёт значение в data/bindings.json — туда же смотрит менеджер профилей."""
        self.data[field] = value
        if self.bindings is None:
            return
        try:
            self.bindings.set_field(self.login, field, value)
        except Exception as exc:  # noqa: BLE001 — запись в память не должна ронять прогон
            self.log.debug("Не удалось сохранить %s в bindings.json: %s", field, exc)
